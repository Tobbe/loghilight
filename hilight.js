function Highlighter() {
	...
	this.highlight = function (logText, wordsToHighlight) {
		hitlist = that.findMatches(logText, wordsToHighlight);
		...
		hitlist.forEach(function(hit, index) {
			output += that.htmlEncode(logText.substring(previousHit.index, hit.index)) + hit.text;
			previousHit = hit;
		});
	...
	this.findMatches = function (logText, wordsToHighlight) {
		...
		if (logText != "" && logLines[0] != "" && logText[logLines[0].length + 1] == '\n' || logText[logLines[0].length + 1] == '\r') {
			newlineCharCount = 2;
		}

		if (wordsToHighlight && wordsToHighlight != '') {
	...
	this.htmlEncodeChar = function(htmlChar) {
		switch (htmlChar) {
			case '<':
				return "&lt;";
			case '>':
